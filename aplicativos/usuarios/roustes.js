// aplicativos/usuarios/routes.js
...
module.exports = function (supabase, supabaseAdmin) {
    ...
    // Lista canônica dos módulos (mesma do portal/routes.js).
    // Serve para validar o que o admin pode liberar.
    const VALID_MODULES = [
    ...
    router.get('/', requireAdmin, async (req, res) => {
        try {
            const { data, error } = await supabaseAdmin
                .from('profiles')
                .select('id, username, name, contact_email, sector, is_admin, is_active, apps, created_at')
                .order('name');
    ...
    router.post('/', requireAdmin, async (req, res) => {
        const { username, name, sector, password, is_active, contact_email, apps } = req.body;
        ...
        // Sanitiza apps: apenas ids válidos, ou lista vazia
        const cleanApps = Array.isArray(apps)
            ? apps.filter(a => VALID_MODULES.includes(a))
            : [];
    ...
    router.put('/:id', requireAdmin, async (req, res) => {
        const { name, sector, password, is_active, contact_email, apps } = req.body;
        const { id } = req.params;
        ...
        try {
            ...
            if (apps !== undefined) {
                const cleanApps = Array.isArray(apps)
                    ? apps.filter(a => VALID_MODULES.includes(a))
                    : [];
                updates.apps = (sector === 'Administrador') ? null : cleanApps;
            }
    ...
    // ─── MÓDULOS DISPONÍVEIS (para o modal) ───────────────────
    // Retorna a lista canônica para o front montar os checkboxes.
